...
def window():
    ...
    def create_button(parent, text, command, color):
        ...
        def on_enter(e):
            btn.config(bg=color, fg=bg_color)
    ...
    deny_button = create_button(btn_container, "Deny", deny_action, warning_color)
    deny_button.pack(side=tk.LEFT, padx=10)
    
    allow_button = create_button(btn_container, "Allow", allow_action, success_color)
